...
func (g *Game) Play(ai AI) int {
	...
	for i := 0; i < g.nHands; i++ {
		shuffled := false
		if len(g.deck) < min {
			g.deck = deck.New(deck.Deck(g.nDecks), deck.FYShuffle)
			shuffled = true
		}
...
// Soft returns true if the score of a hand is a soft score
...
// Blackjack returns true if a hand starts with a blackjack
